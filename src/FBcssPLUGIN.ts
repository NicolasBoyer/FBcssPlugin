...
import { Concat, ensureUserPath, isStylesheetExtension, write } from './Utils'
...
const ensureCSSExtension = (file: File | string): string => {
    const str = file instanceof File ? file.info.fuseBoxPath : file
...
export class CSSPluginClass implements Plugin {
    ...
    public inject(file: File, options: any, alternative?: boolean): string {
        ...
        const result = options.inject !== false && options.inject(ensureCSSExtension(file)) !== false ? `${this.getFunction()}("${resolvedPath}");` : ''
        if (alternative) {
    ...
    public transformGroup(group: File) {
        ...
        const options = group.groupHandler.options || {}
        const cssContents = concat.content

        // writing
        if (options.outFile) {
            const outFile = ensureUserPath(ensureCSSExtension(options.outFile))
    ...
    public async transform(file: File) {
        ...
        if (outFileFunction) {
            ...
            const safeContents = JSON.stringify(file.contents)
            file.sourceMap = undefined
